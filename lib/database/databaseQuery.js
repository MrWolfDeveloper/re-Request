...
database = {
	...
	makeDatabaseQuery: (Data, CallBack) => {
		...
		if (!Data.DatabaseName) Data.DatabaseName = database.DatabaseName;

		const { DatabaseName, DBQueryMethod, MethodData } = Data;

		database[DBQueryMethod](MethodData, (Status, ReturnData, Error) => {
	...
	insert: (MethodData, CallBack) => {
		...
		var Models = require(`${__dirname}/schemas/${ModelName}.js`);
	...
	getCollectionList: (CallBack) => {
		//  Connect To Database
		var DatabaseConnection = Mongoose.connect(`mongodb://localhost:27017/${database.DatabaseName}`);
