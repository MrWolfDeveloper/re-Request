...
saveData = {
		...
		if (options.saveType === 'mongodb') {
			databaseQuery.makeDatabaseQuery(
				...
				(status, returnData, returnError) => {
					...
					callback(null, saveObject.requestID);
				}
			);
		} else if (options.saveType === 'json') {
			try {
				saveData.saveJSON(options.savePath, saveObject);
				callback(null, saveObject.requestID);
			} catch (returnError) {
				callback(new Error(returnError));
			}
	...
	update: (data, callback) => {
		if (!data.options) {
			// Set default options
			data.options = {};
			data.options = { saveType: 'json', savePath: saveData.defaultSavePath };
		} else {
			// We have not save path for mongodb save
			if (data.options.saveType === 'mongodb') data.options.savePath = '';
			else {
				// Set default path for save json.
				if (!data.options.savePath) data.options.savePath = saveData.defaultSavePath;
			}
		}
		...
		if (data.options.saveType === 'mongodb') {
			databaseQuery.makeDatabaseQuery(
				{
					...
					MethodData: {
						...
						Which: {
							requestID: data.newData.requestID
						}
					}
				},
				(status, returnData, returnError) => {
			...
			const requestInfo = saveData.searchJSONFile(data.newData.requestID, data.options);

			Object.keys(data.newData).map((newDataKey) => {
				JSONData[requestInfo.index][newDataKey] = data.newData[newDataKey];
			});

			// Save new data on file
			fs.writeFileSync(data.options.savePath, JSON.stringify(JSONData), 'utf8');
